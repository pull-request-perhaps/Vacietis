...
(defun make-deref* (type expr)
  ...
  (when (eq type 'function-pointer)
    (return-from make-deref* expr))
...
(defun make-ptr+ (lvalue rvalue l-c-type r-c-type)
  (cond
    ((pointer-to-p r-c-type)
     (let* ((next-type (pointer-to-type r-c-type))
  ...
  (cond
    ((and (pointer-to-p r-c-type)
     ...
     (let* ((next-type (pointer-to-type l-c-type))
       ...
       `(let ((,l-ptr ,lvalue)
              (,r-ptr ,rvalue))
          (truncate (- (c-pointer-offset ,l-ptr)
                       (c-pointer-offset ,r-ptr))
                    ,size))))
    ((pointer-to-p l-c-type)
     (let* ((next-type (pointer-to-type l-c-type))
            (alien-type (alien-type-for next-type))
...
(defmacro define-make-ptr-comparison-operators (options &rest ops)
  (declare (ignore options))
  (let (forms)
    (dolist (op ops)
      (let ((fname (intern (format nil "MAKE-PTR~A" op))))
        (push `(defun ,fname (lvalue rvalue l-c-type r-c-type)
                  (declare (ignore l-c-type r-c-type))
                  (let ((l-ptr (gensym))
                        (r-ptr (gensym)))
                    `(let ((,l-ptr ,lvalue)
                           (,r-ptr ,rvalue))
                       (,',op (+ (sb-sys:sap-int (c-pointer-sap ,l-ptr)) (c-pointer-offset ,l-ptr))
                              (+ (sb-sys:sap-int (c-pointer-sap ,r-ptr)) (c-pointer-offset ,r-ptr))))))
              forms)))
    `(progn ,@forms)))

(define-make-ptr-comparison-operators ()
    < <= > >= ==)
...
(defun make-ptr!= (lvalue rvalue l-c-type r-c-type)
  ...
  (let ((l-ptr (gensym))
    ...
    `(let ((,l-ptr ,lvalue)
           (,r-ptr ,rvalue))
       (not (= (+ (sb-sys:sap-int (c-pointer-sap ,l-ptr)) (c-pointer-offset ,l-ptr))
               (+ (sb-sys:sap-int (c-pointer-sap ,r-ptr)) (c-pointer-offset ,r-ptr)))))))
...
(defmacro vac-override (&body body)
  `(macrolet ((vacietis.c:truncl (x)
              ...
              (vacietis.c:+= (lvalue rvalue)
                ...
                (cond
                  ((and *use-alien-types*
                   ...
                   (make-deref*-setf lvalue `(+ ,lvalue ,rvalue)))
                  (t
                   `(incf ,lvalue ,rvalue))))
              (vacietis.c:-= (lvalue rvalue)
                (cond
              ...
              (vacietis.c::alien-ptr!= (lvalue rvalue l-c-type r-c-type)
                (make-ptr!= lvalue rvalue l-c-type r-c-type))
              (vacietis.c:= (lvalue rvalue &optional l-c-type r-c-type)
                (dbg ".c:=: ~S ~S~%" lvalue rvalue)
                (cond
                  ((and *use-alien-types*
                        (listp lvalue)
